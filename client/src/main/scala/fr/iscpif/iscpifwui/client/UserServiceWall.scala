...
object UserServiceWall {
  def apply(user: User, authentication: LoginPassword) = new UserServiceWall(user, authentication)
}

class UserServiceWall(_user: User, authentication: LoginPassword) {
  val user = Var(_user)
  val services = Seq(
  ...
  val userInfoButton = bs.glyphButton(
      ...
      glyph_settings,
      { () =>  bs.showModal(userEditionPanel.modalID) }
  )
